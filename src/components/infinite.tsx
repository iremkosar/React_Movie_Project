interface InfiniteItem {
    ...
    rating: string;
    image: string;
    number?: number; // opsiyonel bir alan olarak ekledim, gerekirse çıkarabilirsiniz
    year?: string;   // opsiyonel bir alan olarak ekledim, gerekirse çıkarabilirsiniz
  }
  ...
  const  myInfiniteFunction: React.FC<InfiniteProps> = ({ item }) => {
    return (
      <div className="w-full h-full pr-4" key={item.id}>
        <div className="flex flex-row gap-3">
          <p className="font-bold text-6xl leading-10 flex items-center">
            {item.number}
          </p>
          <img
          ...
          <div className="flex flex-col gap-3">
            ...
            <p className="text-black dark:text-white truncate 2xl:w-36 sm:w-20 overflow-hidden overflow-ellipsis font-bold text-size16 leading-6 tracking-wider">
              {item.title}
            </p>
            <div className="flex flex-row gap-2 items-center">
              ...
              <p className="flex flex-row gap-2 text-payGray truncate md:w-14 2xl:w-full overflow-hidden overflow-ellipsis">
  ...
  export default myInfiniteFunction;
